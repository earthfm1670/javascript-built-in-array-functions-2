...
// Start coding here
const locationArr = bills.map((x) => x.location);
const uniqueLocation = locationArr.filter(
  (x, index) => locationArr.indexOf(x) === index
);
const objArr = uniqueLocation.reduce((acc, cur) => {
  acc[cur] = sumBill(bills, cur);
  return acc;
}, {});

function sumBill(arr, name) {
  let solution = 0;
  arr.map((key) => {
